;(function() {
  ...
  Instrument = function(config) {
    ...
    if (this.info.samples !== undefined) {
      if (this.info.sample.filesize !== undefined) {
        this.info.samples.filesize = round(
          this.samplepack.filesize / 1024 / 1024,
          2
        )
      }
    }
  ...
  Instrument.prototype.parse = function() {
    var i,
      ...
      config = this.config,
      noteNameMode =
        config.notename_mode === undefined
          ? sequencer.noteNameMode
          : config.notename_mode,
      me = this
    ...
    for (id in this.mapping) {
      if (this.mapping.hasOwnProperty(id)) {
        ...
        if (this.keyRange === undefined) {
          this.lowestNote =
            noteNumber < this.lowestNote ? noteNumber : this.lowestNote
          this.highestNote =
            noteNumber > this.highestNote ? noteNumber : this.highestNote
        }
    ...
    function createSampleConfig(data) {
      ...
      if (config.release_duration !== undefined) {
        ...
        sampleConfig.release_envelope =
          config.release_envelope || me.releaseEnvelope
  ...
  Instrument.prototype.getInfoAsHTML = function() {
    ...
    if (instrumentInfo.descriptiom !== undefined) {
      instrumentHtml +=
        '<tr><td>info</td><td>' + instrumentInfo.description + '</td></tr>'
    }
    if (instrumentInfo.author !== undefined) {
      instrumentHtml +=
        '<tr><td>author</td><td>' + instrumentInfo.author + '</td></tr>'
    }
    if (instrumentInfo.license !== undefined) {
      instrumentHtml +=
        '<tr><td>license</td><td>' + instrumentInfo.license + '</td></tr>'
    }
    instrumentHtml +=
      '<tr><td>keyrange</td><td>' +
      this.lowestNote +
      '(' +
      sequencer.getFullNoteName(this.lowestNote) +
      ')'
    instrumentHtml +=
      ' - ' +
      this.highestNote +
      '(' +
      sequencer.getFullNoteName(this.highestNote) +
      ')</td></tr>'

    if (instrumentHtml !== '') {
      instrumentHtml =
        '<table><th colspan="2">instrument</th>' + instrumentHtml + '</table>'
      html += instrumentHtml
    }

    if (samplesInfo.description !== undefined) {
      samplepackHtml +=
        '<tr><td>info</td><td>' + samplesInfo.description + '</td></tr>'
    }
    if (samplesInfo.author !== undefined) {
      samplepackHtml +=
        '<tr><td>author</td><td>' + samplesInfo.author + '</td></tr>'
    }
    if (samplesInfo.license !== undefined) {
      samplepackHtml +=
        '<tr><td>license</td><td>' + samplesInfo.license + '</td></tr>'
    }
    if (samplesInfo.compression !== undefined) {
      samplepackHtml +=
        '<tr><td>compression</td><td>' + samplesInfo.compression + '</td></tr>'
    }
    if (samplesInfo.filesize !== undefined) {
      samplepackHtml +=
        '<tr><td>filesize</td><td>' + samplesInfo.filesize + ' MiB</td></tr>'
    }

    if (samplepackHtml !== '') {
      samplepackHtml =
        '<table><th colspan="2">samplepack</th>' + samplepackHtml + '</table>'
      html += samplepackHtml
    }
