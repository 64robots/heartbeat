...
;(function() {
  ...
  function store(assetpack) {
    var occupied = findItem(
        assetpack.localPath,
        sequencer.storage.assetpacks,
        true
      ),
    ...
    if (pack.url !== undefined) {
      ajax({
        ...
        onSuccess: function(data, fileSize) {
          ...
          pack.localPath =
            pack.folder !== undefined
              ? pack.folder + '/' + pack.name
              : pack.name
...
;(function() {
  ...
  AudioEvent = function(config) {
    ...
    if (this.buffer === undefined && this.path === undefined) {
      if (sequencer.debug >= sequencer.WARN) {
        console.warn(
          'please provide an AudioBuffer or a path to a sample in the sequencer.storage object'
        )
      }
      return
    }
    ...
    if (
      ...
      typeString(this.buffer) !== 'audiobuffer'
    ) {
      if (sequencer.debug >= sequencer.WARN) {
    ...
    if (
      ...
      this.sampleOffsetTicks === undefined
    ) {
  ...
  AudioEvent.prototype.update = function() {
    ...
    if (this.sampleOffset === undefined) {
      ...
      pos = this.song.getPosition(
        'millis',
        this.millis + this.sampleOffsetMillis
      )
  ...
  AudioEvent.prototype.setSampleOffset = function(type, value) {
    if (type === 'millis') {
      ...
      if (sequencer.debug >= sequencer.WARN) {
        console.warn(
          'you have to provide a type "ticks" or "millis" and a value'
        )
      }
    }
  }
  ...
  AudioEvent.prototype.moveTo = function() {
    ...
    if (position[0] === 'ticks' && isNaN(position[1]) === false) {
      ...
      if (sequencer.debug >= 1) {
        console.error(
          'The audio event has not been added to a song yet; you can only move to ticks values'
        )
      }
    } else {
...
;(function() {
  ...
  function AudioRecorder(track) {
    ...
    this.worker.onmessage = function(e) {
      ...
      encodeAudioBuffer(
        scope,
        e.data.wavArrayBuffer,
        e.data.interleavedSamples,
        e.data.id
      )
    }
  }
  ...
  function createAudioBuffer(
    ...
    var i,
      ...
      recording = {
        ...
        wav: {
          blob: new Blob([new Uint8Array(wavArrayBuffer)], {
            type: 'audio/wav'
          }),
          base64: base64,
    ...
    if (type === 'new') {
      ...
      recording.planarSamples =
        sequencer.storage.audio.recordings[scope.recordId].planarSamples
      recording.interleavedSamples =
        sequencer.storage.audio.recordings[scope.recordId].interleavedSamples
    }
  ...
  function encodeAudioBuffer(scope, wavArrayBuffer, interleavedSamples, type) {
    ...
    context.decodeAudioData(
      ...
      function(audioBuffer) {
        var base64 = encode64(wavArrayBuffer),
          recording = {
            id: scope.recordId,
        ...
        if (type === 'new') {
          ...
          recording.interleavedSamples =
            sequencer.storage.audio.recordings[
              scope.recordId
            ].interleavedSamples
        }
  ...
  AudioRecorder.prototype.createAudio = function() {
    ...
    var bufferIndexStart = parseInt(
        (this.song.metronome.precountDurationInMillis +
          this.song.audioRecordingLatency) /
          millisPerSample
      ),
      bufferIndexEnd = -1
  ...
  AudioRecorder.prototype.setAudioRecordingLatency = function(
    recordId,
    value,
    callback
  ) {
...
;(function() {
  ...
  getStats = function() {
    ...
    if (typeString(searchPattern) !== 'string') {
      console.error(
        "please provide a search string like for instance get('velocity max bar >= 1 < 8')"
      )
      return -1
    }
    ...
    if (patternLength > 2) {
      ...
      if (patternLength === 6) {
        console.warn(
          'ignoring cruft found in search string, please consult documentation'
        )
      }
...
;(function() {
  ...
  var //import
    ...
    logicalOperatorsRegex = new RegExp(
      ' ' + logicalOperators.replace(/\s+/g, ' | ') + ' '
    ), // → replaces logical operator by a white space
    supportedProperties = {
      bar: 1,
      beat: 1,
      sixteenth: 1,
  ...
  findEvent = function() {
    ...
    if (typeString(args[1]) !== 'string') {
      console.error(
        "please provide a search string like for instance findEvent('beat = 2 AND velocity > 60 < 100');"
      )
      return results
    }
...
*/
  ...
  removeMutualEvents = function(resultSet1, resultSet2) {
    var i,
      maxi = resultSet1.length,
      j,
      maxj = resultSet2.length,
      event,
      eventId,
      addEvent,
      result = []
  ...
  checkCondition = function(
    ...
    switch (operator1) {
      ...
      case '!$=':
        result = !(
          propValue.indexOf(value1) ===
          propValue.length - value1.length
        )
        break
...
*/
  ...
  checkOperators = function(pattern) {
    ...
    if (
      pattern.property === 'noteName' &&
      (check(operator1) || check2(operator1))
    ) {
    ...
    if (
      pattern.property === 'noteName' &&
      (check(operator2) || check2(operator2))
    ) {
      pattern.property = 'noteNumber'
...
;(function() {
  ...
  function store(instrument) {
    ...
    if (
      occupied &&
      occupied.className === 'InstrumentConfig' &&
      action !== 'overwrite'
    ) {
      if (sequencer.debug >= 2) {
  ...
  function load(instrument, callback) {
    if (instrument.url === undefined) {
      instrument.localPath =
        instrument.folder !== undefined
          ? instrument.folder + '/' + instrument.name
          : instrument.name
      callback()
      return
    }
  ...
  sequencer.addInstrument = function(
    config,
    callback,
...
;(function() {
  ...
  KeyEditor = function(song, config) {
    ...
    this.horizontalLine = this.iteratorFactory.createHorizontalLineIterator(
      this
    )
    ...
    if (config.paginate) {
      ...
      if (config.width) {
        ...
        this.tickWidth =
          this.viewportWidth /
          (this.startPosition.ticksPerBar * this.barsPerPage)
        this.width = this.numTicks * this.tickWidth
        this.scrollX = 0
      ...
      this.highestNote = config.highestNote || song.highestNote
      this.pitchRange =
        config.pitchRange || this.highestNote - this.lowestNote + 1
  ...
  KeyEditor.prototype.setBarsPerPage = function(bbp) {
    ...
    this.tickWidth =
      this.viewportWidth / (this.startPosition.ticksPerBar * this.barsPerPage)
    this.viewportTicks = this.viewportWidth / this.tickWidth
    this.width = this.numTicks * this.tickWidth
    ...
    this.currentPage =
      floor(this.song.ticks / (this.barsPerPage * this.song.ticksPerBar)) + 1
    ...
    if (this.song.playing) {
      ...
      this.scrollPosition =
        ((this.viewportWidth / this.barsPerPage) * tmp) / this.viewportWidth
...
*/
  ...
  KeyEditor.prototype.getEventRect = function(event) {
    ...
    var x = this.ticksToX(event.ticks - this.startTicks, false),
  ...
  KeyEditor.prototype.getNoteRect = function(note) {
    //console.log(note.number);
    var x = this.ticksToX(note.ticks - this.startTicks, false), //(note.ticks - this.startTicks) * this.tickWidth,
  ...
  KeyEditor.prototype.moveNote = function(x, y) {
    ...
    var newPitch = this.yToPitch(y).number,
  ...
  KeyEditor.prototype.movePart = function(x, y) {
    ...
    if (newPitch !== oldPitch) {
      this.selectedPart.track.transposePart(
        this.selectedPart,
        newPitch - oldPitch
      )
  ...
  KeyEditor.prototype.getTicksAt = KeyEditor.prototype.xToTicks = function(
    x,
    snap
  ) {
  ...
  KeyEditor.prototype.getPitchAt = KeyEditor.prototype.yToPitch = function(y) {
    ...
    var note =
      this.highestNote -
      round(((y + this.scrollY) / this.height) * this.pitchRange)
    note = createNote(note)
    return note
  }

  KeyEditor.prototype.getXAt = KeyEditor.prototype.ticksToX = function(
    ticks,
    snap
  ) {
  ...
  KeyEditor.prototype.getYAt = KeyEditor.prototype.pitchToY = function(
    noteNumber
  ) {
  ...
  KeyEditor.prototype.getSnapshot = function() {
    ...
    if (this.newNumBars !== this.numBars) {
      ...
      this.endPosition = getPosition(this.song, [
        'barsbeats',
        endBar,
        1,
        1,
      ...
      this.verticalLine.reset(
        getPosition(this.song, ['barsbeats', startBar, 1, 1, 0, true]),
        this.endPosition
      )
...
*/
  ...
  setPageData = function(editor, startBar) {
    ...
    editor.startBar =
      editor.startBar > editor.numBars - editor.barsPerPage
        ? editor.numBars - editor.barsPerPage
        : editor.startBar
    editor.endBar = startBar + editor.barsPerPage
    editor.endBar =
      editor.endBar > editor.numBars ? editor.numBars : editor.endBar
    editor.endBar =
      editor.endBar < editor.barsPerPage ? editor.barsPerPage : editor.endBar
  ...
  checkScrollPosition = function(editor) {
    ...
    if (editor.song.playing && editor.interrupt === false) {
      if (editor.song.ticks >= editor.scrollLimit) {
        dispatchEvent(editor, 'scroll', {
          x: editor.scrollX + editor.viewportWidth
        })
        editor.scrollLimit += editor.viewportWidth / editor.tickWidth
        //editor.currentPage++;
      } else {
        var x =
          floor(editor.song.ticks / editor.viewportTicks) *
          editor.viewportTicks *
          editor.tickWidth
        if (editor.scrollX !== x) {
  ...
  sequencer.protectedScope.addInitMethod(function() {
    ...
    createIteratorFactory =
      sequencer.protectedScope.createKeyEditorIteratorFactory
  })
})()
;(function() {
  'use strict'
...
*/
    ...
    hasNext = function(t) {
      ...
      for (; index < numNotes; index++) {
        ...
        if (editor.paginate) {
          ...
          if (
            nextNote.ticks < startTicks &&
            nextNote.noteOff.ticks > startTicks
          ) {
            newNote = true
  ...
  sequencer.protectedScope.createKeyEditorIteratorFactory = function(
    song,
    editor
  ) {
...
;(function() {
  ...
  function createEvents(metronome, startBar, endBar, id) {
    ...
    for (i = startBar; i <= endBar; i++) {
      ...
      for (j = 0; j < beatsPerBar; j++) {
        noteNumber =
          j === 0
            ? metronome.noteNumberAccented
            : metronome.noteNumberNonAccented
        noteLength =
          j === 0
            ? metronome.noteLengthAccented
            : metronome.noteLengthNonAccented
        velocity =
          j === 0 ? metronome.velocityAccented : metronome.velocityNonAccented

        noteOn = sequencer.createMidiEvent(ticks, 144, noteNumber, velocity)
        noteOff = sequencer.createMidiEvent(
          ticks + noteLength,
          128,
          noteNumber,
          0
  ...
  Metronome.prototype.createPrecountEvents = function(precount) {
    ...
    this.precountEvents = createEvents(
      this,
      this.song.bar,
      endPos.bar - 1,
      'precount'
...
;(function() {
  ...
  MidiEvent = function(args) {
    ...
    if (typeString(args[0]) === 'midimessageevent') {
      ...
      typeString(args[0]) === 'number' &&
      typeString(args[1]) === 'number'
    ) {
      data = [args[0], args[1]]
  ...
  MidiEvent.prototype.setPitch = function(pitch) {
    if (this.type !== 0x80 && this.type !== 0x90) {
      if (sequencer.debug >= 1) {
        console.error(
          'you can only set the pitch of note on and note off events'
        )
      }
      return
    }
  ...
  MidiEvent.prototype.moveTo = function() {
    ...
    if (position[0] === 'ticks' && isNaN(position[1]) === false) {
      ...
      if (sequencer.debug >= 1) {
        console.error(
          'The midi event has not been added to a song yet; you can only move to ticks values'
        )
      }
    } else {
      position = this.song.getPosition(position)
...
;(function() {
  ...
  function parse(midifile, buffer, callback) {
    ...
    if (
      sequencer.overrulePPQ === true &&
      isNaN(sequencer.defaultPPQ) === false &&
      sequencer.defaultPPQ > 0
    ) {
        ...
        switch (event.subtype) {
          ...
          case 'noteOn':
            ...
            /*
                        ...
                        */
            parsed.push(
              createMidiEvent(tmpTicks, 0x90, event.noteNumber, event.velocity)
            )
            break
          ...
          case 'noteOff':
            ...
            /*
                        ...
                        */
            parsed.push(
              createMidiEvent(tmpTicks, 0x80, event.noteNumber, event.velocity)
            )
            break
          ...
          case 'timeSignature':
            ...
            if (tmpTicks === ticks && lastType === type) {
              if (sequencer.debug >= 3) {
                console.info(
                  'time signature events on the same tick',
                  j,
                  tmpTicks,
                  event.numerator,
                  event.denominator
          ...
          case 'controller':
            ...
            /*
                        ...
                        */
            parsed.push(
              createMidiEvent(tmpTicks, 0xb0, event.controllerType, event.value)
            )
    ...
    ajax({
      ...
      onSuccess: function(data) {
        if (midifile.responseType === 'json') {
          ...
          midifile.localPath =
            midifile.folder !== undefined
              ? midifile.folder + '/' + midifile.name
              : midifile.name
  ...
  MidiFile = function(config) {
    ...
    if (this.url !== undefined) {
      this.responseType =
        this.url.indexOf('.json') === this.url.lastIndexOf('.')
          ? 'json'
          : 'arraybuffer'
    } else {
      if (this.name === undefined && this.folder === undefined) {
        ...
        this.localPath =
          this.folder !== undefined ? this.folder + '/' + this.name : this.name
      }
    }
  }
...
;(function() {
  ...
  MidiNote = function(args) {
    ...
    if (numArgs === 1) {
      ...
      if (
        on.className === 'MidiEvent' &&
        off & (off.className === 'MidiEvent')
      ) {
...
;(function() {
  ...
  function createStream(buffer) {
    ...
    function readInt16() {
      var result = (buffer[position] << 8) + buffer[position + 1]
      position += 2
      return result
    }
...
;(function() {
  ...
  function write(song) {
    ...
    byteArray = byteArray.concat(
      trackToBytes(song.timeEvents, song.durationTicks, 'tempo')
    )
  ...
  createNote = function() {
    ...
    if (numArgs === 1 && !isNaN(arg0)) {
      ...
      if (!data) {
        error =
          arg0 +
          ' is not a valid note name, please use letters A - G and if necessary an accidental like #, ##, b or bb, followed by a number for the octave'
      } else {
        noteName = data[0]
        octave = data[1]
      ...
      if (!data) {
        error =
          arg0 +
          ' is not a valid note name, please use letters A - G and if necessary an accidental like #, ##, b or bb'
      } else {
        noteName = data[0]
        octave = data[1]
        noteNumber = getNoteNumber(noteName, octave)
      ...
      numArgs === 2 &&
      typeString(arg0) === 'string' &&
      typeString(arg1) === 'string'
    ) {
      data = checkNoteName(arg0)
      if (!data) {
        error =
...
*/
  ...
  Object.defineProperty(sequencer, 'ENHARMONIC_SHARP', {
    value: 'enharmonic-sharp'
  })
  Object.defineProperty(sequencer, 'ENHARMONIC_FLAT', {
    value: 'enharmonic-flat'
  })
...
;(function() {
  ...
  getEvent = function(data, part) {
    ...
    if (!data) {
      ...
      data.className === 'MidiEvent' ||
      data.className === 'AudioEvent'
    ) {
      // new event
      event = data
  ...
  addEvents = function(args, part, relative) {
    ...
    for (i = 0; i < maxi; i++) {
      ...
      if (
        e.type === sequencer.END_OF_TRACK ||
        (e.className !== 'MidiEvent' && e.className !== 'AudioEvent')
      ) {
        continue
      }
  ...
  Part.prototype.addEvent = Part.prototype.addEvents = function() {
    //events
  ...
  Part.prototype.addEventsRelative = function() {
    //events
    var args = getEventsAndConfig(arguments, this)
    addEvents(args, this, true)
  }

  Part.prototype.removeEvent = Part.prototype.removeEvents = function() {
    //events
    var args = getEventsAndConfig(arguments, this)
    if (args === false) {
      return false
  ...
  Part.prototype.moveEvent = Part.prototype.moveEvents = function() {
    //events, ticks
    var args = getEventsAndConfig(arguments, this)
    //console.log(args)
  ...
  Part.prototype.moveAllEvents = function(ticks) {
    //events, ticks
    //console.log(args)
  ...
  Part.prototype.transposeEvents = function() {
    //events, semi
    var args = getEventsAndConfig(arguments, this)
  ...
  Part.prototype.transposeNote = function(note, semi) {
    ...
    transposeEvents(
      { events: [note.noteOn, note.noteOff], config: [semi] },
      this
    )
  }
  /*
...
*/
  ...
  Part.prototype.update = function() {
    ...
    for (i = 0, maxi = this.notes.length; i < maxi; i++) {
      ...
      if (
        note.noteOn.state === 'removed' ||
        (note.noteOff !== undefined && note.noteOff.state === 'removed')
      ) {
        note.state = 'removed'
        this.dirtyNotes[note.id] = note
        delete this.notesById[note.id]
      } else if (
        note.noteOn.state === 'changed' ||
        (note.noteOff !== undefined && note.noteOff.state === 'changed')
      ) {
        note.state = 'changed'
        this.dirtyNotes[note.id] = note
      }
  ...
  Playhead.prototype.calculate = function() {
    ...
    for (i = this.eventIndex; i < this.numEvents; i++) {
      ...
      if (event[this.unit] <= this.currentValue) {
        ...
        if (
          event.type !== sequencer.MIDI_NOTE &&
          event.type !== sequencer.DUMMY_EVENT
        ) {
    ...
    if (this.type.indexOf('events') !== -1 || this.type.indexOf('all') !== -1) {
      ...
      for (i = this.activeEvents.length - 1; i >= 0; i--) {
        ...
        /*
                ...
                */
        if (
          event.state.indexOf('removed') === 0 ||
          this.song.eventsById[event.id] === undefined
        ) {
        ...
        if (
          event[this.unit] <= this.currentValue &&
          event[this.unit] > this.currentValue - range
        ) {
    ...
    if (this.type.indexOf('notes') !== -1 || this.type.indexOf('all') !== -1) {
      ...
      for (i = this.activeNotes.length - 1; i >= 0; i--) {
        ...
        //}
        if (
          note.noteOn.state.indexOf('removed') === 0 ||
          this.song.notesById[note.id] === undefined
        ) {
        ...
        if (note.noteOff === undefined) {
          if (sequencer.debug) {
            console.warn(
              'note with id',
              note.id,
              'has no noteOff event',
              note.noteOn.track.name
            )
          }
          continue
        }
        ...
        if (
          note.noteOn[this.unit] <= this.currentValue &&
          note.noteOff[this.unit] > this.currentValue
        ) {
          //note.active = true;
    ...
    if (this.type.indexOf('parts') !== -1 || this.type.indexOf('all') !== -1) {
      for (i = this.partIndex; i < this.numParts; i++) {
        ...
        if (part.start[this.unit] <= this.currentValue) {
          // && part.end[this.unit] > this.currentValue){
      ...
      for (i = this.activeParts.length - 1; i >= 0; i--) {
        ...
        //}
        if (
          part.start[this.unit] <= this.currentValue &&
          part.end[this.unit] > this.currentValue
        ) {
          newParts.push(part)
        }
...
;(function() {
  ...
  barsToMillis = function(song, position, beos) {
  ...
  barsToTicks = function(song, position, beos) {
    // beos = beyondEndOfSong
    position = ['barsbeats'].concat(position)
  ...
  getPositionData = function(song) {
    ...
    switch (returnType) {
      ...
      case 'barsandbeats':
        ...
        positionData.barsAsString =
          bar + ':' + beat + ':' + sixteenth + ':' + tickAsString
        break
    ...
    for (i = events.length - 1; i >= 0; i--) {
      ...
      if (event.type !== 128) {
        ...
        if (event.midiNote !== undefined) {
          ...
          quantizeHistory.tracks[track.id].quantizedEvents.push(
            event.midiNote.noteOff
          )
        }
      }
    }
...
;(function() {
  ...
  function store(samplepack) {
    var occupied = findItem(
        samplepack.localPath,
        sequencer.storage.samplepacks,
        true
      ),
  ...
  sequencer.addSamplePack = function(
    config,
    callback,
    callbackAfterAllTasksAreDone
  ) {
    ...
    callbackAfterAllTasksAreDone =
      callbackAfterAllTasksAreDone === undefined
        ? false
        : callbackAfterAllTasksAreDone
    ...
    for (i = 0; i < this.numAudioEvents; i++) {
      ...
      if (event.millis < millis && event.endMillis > millis) {
        ...
        event.time =
          this.startTime +
          event.millis -
          this.songStartMillis +
          event.playheadOffset
      ...
      if (event.millis < this.maxtime) {
        ...
        if (event.type === 144 || event.type === 128) {
          if (
            ...
            event.midiNote.noteOff !== undefined
          ) {
            if (event.type === 144) {
          ...
          if (this.scheduledAudioEvents[event.id] !== undefined) {
            ...
            if (
              audioEvent.sample !== undefined &&
              audioEvent.sample.source !== undefined
            ) {
        ...
        if (track.routeToMidiOut === false) {
          ...
          objectForEach(track.midiOutputs, function(midiOutput) {
            if (
              ...
              midiOutput.send(
                [event.type + channel, event.data1, event.data2],
                event.time
              )
...
;(function() {
  ...
  heartbeat = function(timestamp) {
    ...
    if (r >= 10) {
      ...
      // }
      if (
        diff > sequencer.bufferTime &&
        sequencer.autoAdjustBufferTime === true
      ) {
        if (sequencer.debug) {
          console.log(
            'adjusted buffertime:' + sequencer.bufferTime + ' -> ' + diff
          )
        }
    ...
    loop = function(data, i, maxi, indentation) {
      for (i = 0; i < maxi; i++) {
        ...
        if (arg === undefined) {
          ...
          arg.className === 'MidiEvent' ||
          arg.className === 'AudioEvent'
        ) {
          if (bpm === undefined) {
            part = arg.part
            if (part !== undefined) {
...
;(function() {
  ...
  Song = function(config) {
    ...
    this.autoSize =
      config.autoSize === undefined ? true : config.autoSize === true
    this.playbackSpeed = 1
    this.defaultInstrument =
      config.defaultInstrument || sequencer.defaultInstrument
    this.recordId = -1
    ...
    if (config.timeEvents && config.timeEvents.length > 0) {
      ...
      if (this.timeSignatureEvent === undefined) {
        this.timeSignatureEvent = createMidiEvent(
          0,
    ...
    if (
      config.timeEvents !== undefined &&
      (config.nominator !== undefined || config.denominator !== undefined)
    ) {
      if (
        this.nominator !== config.nominator ||
        this.denominator !== config.denominator
      ) {
        this.setTimeSignature(
          config.nominator || this.nominator,
          config.denominator || this.denominator,
          false
        )
      }
    }
    ...
    if (config.events || config.parts || config.tracks) {
      ...
      this.lastEvent = createMidiEvent([
        this.bars * this.ticksPerBar,
        sequencer.END_OF_TRACK
      ])
    }
    //console.log('update');
    this.update(true)
...
*/
  ...
  Song.prototype.remove = function() {
    console.warn(
      'Song.remove() is deprecated, please use sequencer.deleteSong()'
    )
  ...
  Song.prototype.setAudioRecordingLatency = function(
    recordId,
    value,
    callback
  ) {
  ...
  Song.prototype.startRecording = Song.prototype.record = function(precount) {
    ...
    if (this.playing) {
      ...
      if (precount === undefined) {
        ...
        this.recordStartMillis =
          this.millis - this.metronome.precountDurationInMillis
...
*/
  ...
  Song.prototype.setLeftLocator = function() {
    ...
    this.doLoop = this.illegalLoop === false && this.loop === true
    this.loopDuration =
      this.illegalLoop === true ? 0 : this.loopEnd - this.loopStart
  ...
  Song.prototype.addTrack = function() {
    ...
    if (typeString(arg0) === 'array' || numArgs > 1) {
      console.warn(
        'please use addTracks() if you want to get more that one tracks'
      )
      args = [arg0]
    }
  ...
  Song.prototype.getPart = function() {
    ...
    if (args.length > 1) {
      console.warn(
        'please use getParts() if you want to get more that one part'
      )
    }
  ...
  Song.prototype.removeTrack = function() {
    ...
    if (args.length > 1) {
      console.warn(
        'please use removeTracks() if you want to remove more that one tracks'
      )
    }
  ...
  Song.prototype.updateTimeSignatureEvent = function(
    event,
    nominator,
    denominator
  ) {
...
*/
  ...
  Song.prototype.resetTimeSignature = function(nominator, denominator) {
    var firstTimeSignatureEvent = getTimeEvents(
        sequencer.TIME_SIGNATURE,
        this
      )[0],
      timeEvents = this.timeEvents,
...
;(function() {
  ...
  var // satisfy jslint
    ...
    supportedOperatorsRegex = new RegExp(
      ' ' + supportedOperators.replace(/\s+/g, ' | ') + ' '
    ),
    //private
    getEvents,
...
*/
  ...
  FollowEvent.prototype.callListenersPositionRepetitive = function(
    positionType
  ) {
    var listener,
  ...
  FollowEvent.prototype.callListenersPositionConditionalSimple = function(
    positionType,
    value
  ) {
    var listener,
      ...
      operator,
      call = false,
      listenerIds = this.allListenersByType.position.conditional_simple[
        positionType
      ],
      me = this
  ...
  FollowEvent.prototype.callListenersPositionConditionalComplex = function(
    positionType,
    value
  ) {
    var listener,
      ...
      operator1,
      operator2,
      listenerIds = this.allListenersByType.position.conditional_complex[
        positionType
      ],
      me = this
  ...
  FollowEvent.prototype.addEventListener = function(type, data, callback) {
    ...
    if (typeString(callback) !== 'function') {
      console.error(
        callback,
        'is not a function; please provide a function for callback'
      )
      return -1
    }
...
*/
    ...
    if (supportedTimeEvents[type] !== 1) {
      console.error(
        type,
        'is not a supported event id, please consult documentation'
      )
      return -1
    }
    ...
    switch (type) {
      ...
      case 'millis':
        if (operator1 === undefined || operator1 !== '===') {
          console.error(
            type,
            "can only be used conditionally with the operators '===', '==' or '='"
          )
          return -1
        }
        ...
        if (operator2) {
          console.warn(
            'this position event type can only be used with a single condition, ignoring second condition'
          )
        }
        break
    ...
    if (operator1 && supportedOperators.indexOf(operator1) === -1) {
      console.error(
        operator1,
        'is not a supported operator, please use any of',
        supportedOperators
      )
      return -1
    }
    ...
    if (
      operator1 &&
      operator2 &&
      checkOperatorConflict(operator1, operator2) === false
    ) {
      console.error(
        "you can't use " + operator1 + ' together with ' + operator2
      )
      return -1
    }
      ...
      for (i = ids.length - 1; i >= 0; i--) {
        ...
        if (this.allListenersById[id] !== undefined) {
          ...
          if (type === 'position') {
            ...
            listenerIds = this.allListenersByType[type][subtype][
              listener.position_type
            ]
            ...
            this.allListenersByType[listener.type][listener.subtype][
              listener.position_type
            ] = [].concat(filteredListenerIds)
            ...
            this.allListenersByType.event[subtype][eventId] = [].concat(
              filteredListenerIds
            )
            delete this.allListenersById[id]
...
*/
      ...
      switch (type) {
        case 'position':
          if (dataType === 'string') {
            ...
            listener = this.allListenersById[id]
            // reference to an array of all the listeners bound to this event type
            listenerIds = this.allListenersByType[listener.type][
              listener.subtype
            ][listener.position_type]
            // loop over listeners and filter the one that has to be removed
            for (i = listenerIds.length - 1; i >= 0; i--) {
          ...
          if (dataType === 'string') {
            ...
            for (eventId in eventIds) {
              if (eventIds.hasOwnProperty(eventId)) {
                ...
                // add the filtered array back
                this.allListenersByType.event.searchstring[eventId] = [].concat(
                  filteredListenerIds
                )
              }
            }
            ...
            if (this.allListenersByType.event.instance[id] !== undefined) {
              ...
              this.allListenersByType.event.searchstring[id] !== undefined
            ) {
              type = 'searchstring'
            ...
            for (i = listenerIds.length - 1; i >= 0; i--) {
              ...
              this.allListenersByType.event[type][
                listener.event.id
              ] = [].concat(filteredListenerIds)
            }
  ...
  FollowEvent.prototype.resetAllListeners = function() {
    var listeners = this.allListenersById,
      key,
      listener
  ...
  getEvents = function(type, data) {
    ...
    if (
      ...
      console.error(
        data,
        " is not valid data for event type 'event', please consult documentation"
      )
      return -1
    }
    ...
    if (dataType === 'array') {
      for (i = data.length - 1; i >= 0; i--) {
        ...
        if (
          type === 'event' &&
          e.className !== 'MidiEvent' &&
          e.className !== 'AudioEvent'
        ) {
          console.warn(
            'skipping',
            e,
            'because it is not a MidiEvent nor an AudioEvent'
          )
          continue
...
*/
    ...
    eventsMidiAudioMetronome = [].concat(
      midiEvents,
      audioEvents,
      song.metronome.events
    )
...
;(function() {
  ...
  function getTicksAtPosition(position, song) {
    ...
    if (song === undefined && type !== 'ticks') {
      console.error(
        'Track has not been added to a song yet, you can only use tick values'
      )
      return false
    }
  ...
  function removeEvents(args, track) {
    ...
    for (i = tobeRemoved.length - 1; i >= 0; i--) {
      ...
      if (event.track !== undefined && event.track !== track) {
        console.warn(
          "can't remove: this event belongs to track",
          event.track.id
        )
        continue
      }
  ...
  Track.prototype.addPart = Track.prototype.addParts = function() {
    //newParts
  ...
  Track.prototype.movePart = Track.prototype.moveParts = function() {
    //parts, ticks
    var args = getPartsAndConfig(arguments, this)
    moveParts(args, this)
  }

  Track.prototype.movePartTo = Track.prototype.movePartsTo = function() {
    //selectedParts, position
    var args = getPartsAndConfig(arguments, this)
  ...
  Track.prototype.getPartAt = Track.prototype.getPartsAt = function(position) {
    ...
    if (ticks === false) {
      console.error(
        "please provide position as array, for instance: ['barsandbeats',5,1,2,0]"
      )
      return
    }
  ...
  Track.prototype.getPartFromTo = Track.prototype.getPartsFromTo = function(
    from,
    to
  ) {
    ...
    parts.forEach(function(part) {
      if (
        (fromTicks >= part.start.ticks && fromTicks <= part.end.ticks) ||
        (toTicks >= part.start.ticks && toTicks <= part.end.ticks)
      ) {
        selectedParts.push(part)
      }
    })
  ...
  Track.prototype.getPartBetween = Track.prototype.getPartBetween = function(
    from,
    to
  ) {
    var parts = this.parts,
  ...
  Track.prototype.removeEvent = Track.prototype.removeEvents = function() {
    //events
    var args = getEventsAndConfig(arguments, this)
  ...
  Track.prototype.removeEventAt = Track.prototype.removeEventsAt = function(
    position
  ) {
  ...
  function getDefaultInstrumentConfig(track) {
    ...
    if (
      ...
      track.song.defaultInstrument !== undefined
    ) {
      config = findItem(
        track.song.defaultInstrument,
        sequencer.storage.instruments
      )
  ...
  Track.prototype.setMidiInput = function(id, flag) {
    ...
    if (id === 'all') {
      availableInputs =
        this.song !== undefined ? this.song.midiInputs : sequencer.midiInputs
  ...
  Track.prototype.encodeAudioRecording = function(
    recordId,
    type,
    bitrate,
    callback
  ...
  Track.prototype.setAudioRecordingLatency = function(
    recordId,
    value,
    callback
  ) {
